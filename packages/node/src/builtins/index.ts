/**
 * @sigurjs/node **builtins** — Result-returning Node/JS globals (no `node:` imports).
 *
 ...
 * `@sigurjs/core` turns that into `Result` / `ResultAsync` so callers extract the
 ...
 * `import { JSON, fetch, URL, Request, Response } from "@sigurjs/node"`.
